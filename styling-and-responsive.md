...
These breakpoints work the opposite way from how most page builders present "mobile / tablet / desktop" controls. Each suffixed value applies **at that width and narrower**, not "that width and up." The base (un-suffixed) value is what shows on the *widest* screens, meaning anything larger than your largest breakpoint.
...
> Other settings (Speed, Autoplay, Arrows, etc.) do **not** have breakpoint versions and apply the same at every screen size. (Gap **does**, and it takes the same `sm:`/`md:`/`lg:` shorthand, or a single `clamp()` value if you'd rather it scale fluidly.)
...
**Sllde Direction** (`data-direction`) takes the shorthand like any other responsive setting, though it's typed by hand, not picked from a dropdown. This is how you get a vertical slider on desktop that turns horizontal on phones:
...
Because activation happens *before* the slider scrolls into view, there's normally nothing to see while it waits, because by the time it's on screen it's already a live slider.
...
> **Older browsers:** if the browser doesn't support `IntersectionObserver`, lazy sliders simply initialize immediately, and nothing breaks.
...
`dwc-slider-pro-etch/assets/dwc-slider-pro.css` also defines a set of site-wide theme defaults (fonts, background/surface/text/accent/border colors, arrow/dot/progress base colors and sizes, transition timing). These are fallback values used by the plugin's own demo/reference markup, and they're not exposed as component props and aren't something you'd change per slider.
