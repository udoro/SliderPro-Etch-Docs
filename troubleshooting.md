...
| Slider shows every item at once / won't move when built from a loop | The Etch **Loop** is on the content *inside* a slide, so everything lands in one slide. Put the loop on the **DWC Slide** itself — one loop pass = one slide. See [Tips & Recipes](tips.md#building-slides-from-an-etch-loop). |
| Images or thumbnails stretch or change size/aspect ratio | A layout issue, not the slider: images are showing at their natural size. Make each image fill its slide with `object-fit: cover`, then give the Slider a size — a **Slider Height** or **Aspect Ratio** for a main slider, but a **fixed Slider Height** (e.g. `80px`) for a **thumbnail** strip (Aspect Ratio sizes the whole strip, not each tile). See [Tips & Recipes](tips.md#making-images-fill-a-slide-consistently). |
