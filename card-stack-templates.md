...
- **A second Slider Stack on the same page needs its own card class.** A slider looks for your class anywhere on the page, so a straight duplicate leaves both sliders controlling both stacks, and a click on one card moves both. On the copied cards: add a new class such as `slider-stack-2__card`, then remove the old one from them. **Don't rename the class**, because renaming changes it on the first stack too. Copy the CSS across to the new class, then set the second slider's **Sync Custom Element** to `.slider-stack-2__card`. Deck and Fall never need this: they have no slider, and each wrapper only controls the cards inside it.
